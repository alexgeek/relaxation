...
gnuplot <<- EOF
    ...
    set output "time.png"
    plot "time.${TIMES[0]}.log" title "Speedup (${TIMES[0]})" smooth unique lw 3, \
      "time.${TIMES[1]}.log" title "Speedup (${TIMES[1]})" smooth unique lw 2, \
      "time.${TIMES[2]}.log" title "Speedup (${TIMES[2]})" smooth unique lw 2
EOF
